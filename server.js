...
// ── Create welcome.txt for new users ──────────────────────────
...
Hello, ${USERNAME}! Your personal VPS is ready.
  ...
  • This is your private workspace — only you can access it.
  • Your files are saved automatically to the cloud after every change.
  • Storage limit: 1 GB  (check the bar in the file manager)
  • Session duration: up to 55 minutes per node.

Quick tips:
  - Use the file manager (left panel) to browse, upload & edit files.
  - Type commands in the terminal on the right.
  - Click "Disconnect" to cleanly shut down your node.
  - Deploy a fresh node from the dashboard anytime.

Useful commands to get started:
  ls -la          # list files
  htop            # system monitor  (press q to quit)
  node --version  # check Node.js
  python3 --version

Happy hacking!
— The ABVPS Team @ abvps.gt.tc
`;
  fs.writeFileSync(welcomePath, welcomeContent, "utf8");
  console.log(`[ABVPS] Created welcome.txt for new user: ${USERNAME}`);
}
